...
void zf_model_odresult_out(const odresult_t* odres, int retcnt)
{
    ...
    for(int i = 0; i < retcnt; i++, odres++)
    {
		...
		temp_x = (int)(od_result.res_x1 + od_result.res_x2)*0.5;
		temp_y = (int)(od_result.res_y1 + od_result.res_y2)*0.5;
...
void zf_model_init(void)
{
    ...
    postProcessParams.threshold = 0.3;
...
void zf_model_run(void)
{
    ...
    {
		tof = 'T';
        zf_model_odresult_out(s_odrets, 1);
    }
	if(s_odretcnt == 0 && tof == 'T'){
		tof = 'F';
		zf_model_odresult_out(no_odret, 1);
	}
